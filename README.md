# Customer Segmentation Using K-Means Clustering

This project applies K-means clustering to segment customers based on demographic and behavioral variables. The goal is to identify meaningful customer groups and interpret how these segments can support business decision-making.

## Methods

- Data cleaning and preprocessing in R
- Exploratory data analysis
- K-means clustering
- Elbow method and silhouette analysis for cluster evaluation
- Visualization and interpretation of customer segments

## Tools

- R
- Quarto
- ggplot2
- dplyr
- cluster / factoextra

## Files

- `customer_segmentation_kmeans.qmd` – source code and analysis
- `customer_segmentation_kmeans.html` – rendered report

## Key Takeaways

The analysis demonstrates how unsupervised learning can be used to structure customer data, identify patterns and support more targeted business decisions.
